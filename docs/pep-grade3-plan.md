...
记录日期：2026-09-23  
状态：P0–P5 已落地（2026-09-23；读写任务/语法引擎仍暂缓；TTS → 127.0.0.1:7860）  
...
### P5 — 按年级轨迹加厚单元 ✅ 2026-09-23

依据 `docs/人教版英语.md` 能力表（三听读模仿 → 四对话情境 → 五观点习惯 → 六比较级/过去时），分层加厚：

| 年级 | 词/单元 | 句型/单元 | 内容侧重 |
|------|---------|-----------|----------|
| 三 | 14 | 5 | 场景词补齐、问答模仿 |
| 四 | 15 | 5 | can/must/should、时间与购物 |
| 五 | 16 | 5 | 情感/习惯/规则/旅行表达 |
| 六 | 16 | 6 | 比较级、过去时句型强化 |

- 优先复用 Ogden 词典；新增 28 个 `pep_extra`（近义词+IPA 齐全）
- 单测范围放宽至 words 8–18、phrases 3–6；`unitsThickenByGradeTrajectory` 锁定分层目标
- 新增词/句型本地音频：`generate_pep_audio.py`（跳过已有文件）
